...
import cn.waicaibao.user.entity.base.BaseUser;
import cn.waicaibao.util.Convert;
...
//		return Picture.getRet() + AppConfig.RESOURCE_APP_ROOT + USER_RESOURCE;
	...
	/**
	 * 获得显示用户名 昵称不为空则返回昵称，否则返回隐藏4-7位的用户名
	 * 
	 * @return
	 */
	public String getShowName() {
		return StringUtils.isNotBlank(getCnName()) ? getCnName() : Convert.toHiddenPhone(getName());
	}
	...
	public String getHideName() {
		return StringUtils.isNotBlank(getCnName()) ? Convert.toHiddenPhone(getCnName()) : Convert.toHiddenPhone(getName());
	}
